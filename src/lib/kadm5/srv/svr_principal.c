...
make_ks_from_key_data(krb5_context context, kadm5_key_data *key_data,
                      int n_key_data, krb5_key_salt_tuple **ks_tuple)
{
    int i;

    *ks_tuple = calloc(n_key_data, sizeof(**ks_tuple));
    if (*ks_tuple == NULL)
        return ENOMEM;

    for (i = 0; i < n_key_data; i++) {
        (*ks_tuple)[i].ks_enctype = key_data[i].key.enctype;
        (*ks_tuple)[i].ks_salttype = key_data[i].salt.type;
    }
    return 0;
}
                         ...
                         int n_key_data)
{
    ...
    if (hist_princ && /* this will be NULL when initializing the database */
        ((krb5_principal_compare(handle->context,
                                 principal, hist_princ)) == TRUE))
        return KADM5_PROTECT_PRINCIPAL;

    /* we accept keys for one kvno only */
    ...
    /* a kvno of -1 means we need to pick the next kvno */
    if (kvno == 0) {
        for (i = 0; i < kdb->n_key_data; i++) {
            if (kdb->key_data[i].key_data_kvno > kvno) {
                kvno = kdb->key_data[i].key_data_kvno;
    ...
    if (keepold) {
        n_new_key_data = kdb->n_key_data + n_key_data;
    } else {
        n_new_key_data = kdb->n_key_data;
    }
    ...
    if (keepold) {
        for (i = 0; i < kdb->n_key_data; i++) {
            new_key_data[n_new_key_data] = kdb->key_data[i];
            memset(&kdb->key_data[i], 0, sizeof(krb5_key_data));
            n_new_key_data++;
        }
    }

    /* finally replace kdb->key_data with the new keys */
    cleanup_key_data(handle->context, kdb->n_key_data, kdb->key_data);
    ...
    if ((ret != KADM5_OK) && (new_key_data != NULL)) {
