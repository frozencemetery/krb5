 ...
 */
static krb5_error_code
derive_random_sp800_108_counter_hmac(const struct krb5_hash_provider *hash,
                                     krb5_key inkey, krb5_data *outrnd,
                                     const krb5_data *in_constant)
{
    ...
    ret = krb5int_hmac(hash, inkey, iov, 4, &prf);
    if (!ret)
                      ...
                      const krb5_data *in_constant, enum deriv_alg alg)
{
    ...
    case DERIVE_SP800_108_HMAC:
        return derive_random_sp800_108_counter_hmac(hash, inkey, outrnd,
                                                    in_constant);
    default:
        return EINVAL;
    }
